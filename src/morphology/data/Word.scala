  ...
  def toDistinctLabels(sliceSize: Int): Array[Array[Label]] = {
    originalAnalyses.zipWithIndex.map { case (analysis, analysisIndex) => {
      ...
      val stemLabel = Label(index, text, stem, analysis, stem)
        ...
        .setTrueLabel(analysisIndex)
        .setLemmaLabel(true)
        .setAnalysisIndex(analysisIndex)
        .setTotalCount(1)
    ...
    def toContraintLabels(sliceSize: Int): Array[(Label, Label)] = {
      val ambiguityCount = originalAnalyses.length
      originalAnalyses.zipWithIndex.flatMap { case (crrAnalysis, crrAnalysisIndex) => {
        val tags = crrAnalysis.split("\\+")
        val stem = tags.head
        val crrLabels = tags.sliding(sliceSize, 1).map(tagSeq => {
          Label(index, text, stem, crrAnalysis, tagSeq.mkString("+"))
            .setAmbiguousCount(ambiguityCount).setAnalysisIndex(crrAnalysisIndex)
        })
        originalAnalyses.zipWithIndex.filter(_._2 != crrAnalysisIndex).flatMap { case (otherAnalysis, otherAnalysisIndex) => {
          val tags = otherAnalysis.split("\\+")
          val stem = tags.head
          val otherLabels = otherAnalysis.split("\\+").sliding(sliceSize, 1).map(tagSeq => {
            Label(index, text, stem, otherAnalysis, tagSeq.mkString("+"))
              .setAmbiguousCount(ambiguityCount).setAnalysisIndex(otherAnalysisIndex)
          })
          crrLabels.flatMap(crrLabel => otherLabels.map(otherLabel => {
            (crrLabel, otherLabel)
          }))
        }}
      }}
